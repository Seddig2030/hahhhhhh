...
PLACEMENT=[
('She ___ a teacher.',['is','am','are','be'],0),('They ___ from Jordan.',['is','am','are','was'],2),
('I ___ to the market yesterday.',['go','goes','went','going'],2),('By the time we arrived, the film ___ already started.',['has','have','had','was'],2),
('If I ___ more time, I would learn more.',['have','had','has','will have'],1),('The report ___ written by the research team.',['is','was','did','has'],1),
('Ostensibly is closest in meaning to:',['secretly','apparently','rarely','urgently'],1),('If I ___ studied harder, I would have passed.',['have','had','has','did'],1)]
...
class Landing(Base):
 def on_pre_enter(self):
  ...
  self.button(c,'ابدأ اختبار تحديد المستوى',lambda:setattr(self.manager,'current','placement'),True); self.button(c,'أدخل مباشرة للتعلم',self.direct); self.label(c,'المفردات الكبيرة تُحمّل مرة واحدة ثم تعمل من الهاتف بدون إنترنت.',13); self.add_widget(r)
 def direct(self):
  d=progress(); d['level']=d['level'] or 'A1'; savep(d); self.manager.current='dashboard'

class Placement(Base):
 def on_pre_enter(self): self.i=0; self.score=0; self.render()
 def render(self):
  self.clear_widgets(); r,c=self.shell(); q=PLACEMENT[self.i]; self.label(c,f'اختبار المستوى — {self.i+1}/{len(PLACEMENT)}',14); self.label(c,q[0],21)
  for i,o in enumerate(q[1]): self.button(c,o,lambda i=i:self.answer(i))
  self.add_widget(r)
 def answer(self,i):
  self.score += i==PLACEMENT[self.i][2]; self.i+=1
  if self.i==len(PLACEMENT):
   lvl='A1' if self.score<=1 else 'A2' if self.score<=3 else 'B1' if self.score<=5 else 'B2' if self.score<=6 else 'C1'; d=progress(); d['level']=lvl; savep(d); self.manager.current='dashboard'
  else:self.render()
...
class Lesson(Base):
 def start(self,l):self.lesson=l;self.stage='vocab';self.i=0;self.qi=0;self.score=0;self.queue=list(range(len(l['vocab'])));self.revealed=False;self.render()
 def render(self):
 ...
 def writing(self,c):
  ...
  def go():
   ...
   self.i+=1;self.setstage('grammar') if self.i>=len(self.lesson['vocab']) else self.render()
 ...
 def grammar(self,c):
  ...
  self.button(c,'التالي',lambda:self.setstage('reading' if self.lesson.get('reading') else 'quiz'),True)
 def reading(self,c):
  r=self.lesson['reading'];self.label(c,r['titleAr'],19);self.label(c,r['en'],16);self.label(c,r['ar'],14);self.button(c,'🔊 استمع',lambda:speak(r['en']));self.button(c,'التالي: الاختبار',lambda:self.setstage('quiz'),True)
 def quiz(self,c):
...
class WalkLearn(App):
 def build(self):
  ...
  for cls,name in [(Landing,'landing'),(Placement,'placement'),(Dashboard,'dashboard'),(Vocab,'vocab'),(Word,'word'),(WriteWord,'writeword'),(Grammar,'grammar'),(GramDetail,'gramdetail'),(Lessons,'lessons'),(Lesson,'lesson'),(Progress,'progress')]:sm.add_widget(cls(name=name))
